...
void test_prefix_range(uint64_t start, uint64_t end, const std::string& target_prefix) {
    // Create a dummy address for testing
    account_public_address dummy_address{};
    
    // Convert target prefix to uppercase for case-insensitive comparison
    std::string target_upper = target_prefix;
    ...
    for (uint64_t prefix = start; prefix <= end; prefix++) {
        ...
        if (addr.substr(0, target_prefix.length()) == target_upper) {
...
int main(int argc, char* argv[]) {
    std::string target_prefix = "XEQ";  // Default prefix
    
    // Allow override of target prefix from command line
    if (argc > 1) {
        target_prefix = argv[1];
    }
    
    std::cout << "Searching for address prefix that generates '" << target_prefix 
              << "' addresses...\n";
    
    // Test different ranges of prefixes
    // Start with smaller numbers first as they're more likely to produce shorter addresses
    test_prefix_range(1, 1000000, target_prefix);           // First million
    test_prefix_range(1000000, 10000000, target_prefix);    // Next 9 million
    test_prefix_range(10000000, 100000000, target_prefix);  // Next 90 million if needed
