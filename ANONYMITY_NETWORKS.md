# Anonymity Networks with XEQM

Currently only Tor and I2P have been integrated into XEQM Core. The usage of
...
is made or until `xeqm-d` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `xeqm-wallet` and
`xeqm-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `xeqm-d` the ip/port of a socks proxy provided by a
...
which tells `xeqm-d` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `xeqm-d` the inbound address, network
...
which tells `xeqm-d` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `xeqm-d`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `xeqm-d` localhost port 30000.
...
`xeqm-d` RPC port - which is independent from the configuration for incoming
...
system clock is fairly accurate - many peers on the XEQM network should have
similar timestamps.
...
### Intermittent XEQM Syncing

If a user only runs `xeqm-d` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `xeqm-d` as often as possible to conceal when transactions are being sent.
...
`xeqm-d` currently selects two outgoing connections every 5 minutes for
...
`xeqm-d` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `xeqm-d` should likely disconnect
