...
class Daemon(RPCDaemon):
    ...
    def get_accrued_rewards(self, addresses: list[eth_typing.evm.ChecksumAddress]) -> list[AccruedRewards]:
        ...
        print(balance_array[0])
        print(balance_array[0].keys())
        print(balance_array[0]['address'])
        # print([address, amount in balance_array.items()])
        for balance_item in balance_array:
            item         = AccruedRewards()
            item.address = balance_item["address"]
            item.balance = balance_item["amount"]
            result.append(item)
        return result
